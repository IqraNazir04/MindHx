...
export default function Home() {
  ...
  const [voiceFeatures, setVoiceFeatures] = useState<{ risk_signal?: number; pause_ratio?: number; energy_variability?: number; speaking_rate?: number }>({});
  const [typedText, setTypedText] = useState("");
  const [textSubmitting, setTextSubmitting] = useState(false);
  const [textSubmitError, setTextSubmitError] = useState("");
  const [textSubmitResult, setTextSubmitResult] = useState<{ sentiment: string } | null>(null);
  ...
  async function handleSubmitText() {
    ...
    try {
      ...
      const result = await response.json() as { sentiment: string; crisis_language: boolean };
      ...
      setTextSubmitResult({ sentiment: result.sentiment });
    } catch {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          const voiceResult = await voiceResponse.json() as { risk_signal: number; pause_ratio: number; energy_variability: number; speaking_rate: number };
          setVoiceFeatures({ risk_signal: voiceResult.risk_signal, pause_ratio: voiceResult.pause_ratio, energy_variability: voiceResult.energy_variability, speaking_rate: voiceResult.speaking_rate });
        } catch {
          setVoiceFeatures({});
        }
      };
